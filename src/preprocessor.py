...
class Preprocessor:
    ...
    def _parse_fn(self, filename, label):
        ...
        return clip, label  # ONE HOT ENCODE

    def augment(x):
        x = tf.cast(x, tf.float32)  # idk if needed
        x = tf.divide(x, tf.constant(255.0))  # Normalize
        x = tf.map_fn(lambda x1: tf.image.random_brightness(x1, max_delta=0.2), x)
        x = tf.map_fn(lambda x1: tf.image.random_hue(x1, max_delta=0.5), x)
        ...
        x = tf.map_fn(lambda x1: tf.keras.preprocessing.image.random_shift(x, 0.25, 0.25), x)
        return x